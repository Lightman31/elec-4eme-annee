...
void glcd_print_tab(unsigned char tabAffichag[22][10]);
...
void glcd_print_tab(unsigned char tabAffichage[22][10])
{
    ...
    for(i=0;i<=9;i++)
      {
         for(j=0;j<=21;j++)
           {
              if( tabAffichage[j][i] == 1)
              {
                 glcd_draw_cube(i,j,1);
              }
              if( tabAffichage[j][i] == 0)
              {
